...
class LiveConsoleReporter:
    def __init__(self, console: Console, *, json_output: bool = False, event_jsonl_path: Path | None = None) -> None:
...
class ReporterFactory:
    def __init__(self, console: Console, json_output: bool, event_jsonl_path: Path | None) -> None:
        self.console = console
        self.json_output = json_output
        self.event_jsonl_path = event_jsonl_path
...
def _render_event_text(event: EngineEvent) -> Text:
    ...
    if kind == "suite.finished":
        return Text(
            f"■ Suite finished: status={payload.get('status')} "
    ...
    if kind == "test.finished":
        return Text(f"  ■ Test finished: {payload.get('id')} status={payload.get('status')}")
    ...
    if kind == "response.received":
        return Text(f"      ← Response: {payload.get('statusCode')} in {round(float(payload.get('totalMs') or 0), 2)} ms")
    if kind == "assertions.evaluated":
        return Text(f"      ✓ Assertions: passed={payload.get('passed')} count={payload.get('count')}")
    ...
    if kind == "step.finished":
        return Text(f"    ■ Step finished: {payload.get('id')} status={payload.get('status')}")
